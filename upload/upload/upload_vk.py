...
def upload_to_vk(video_path, description="", title=""):
    ...
    try:
        ...
        # Prepare message
        message = description if description else "✨ Sandra Bullock Daily!\n\n#sandrabullock #hollywood #celebrity #fashion"
        ...
        video = upload.video(
            ...
            name=title or 'Sandra Bullock Daily Video',
...
def main():
    ...
    description = sys.argv[2] if len(sys.argv) > 2 else "Daily Sandra Bullock Moment! ✨"
    title = sys.argv[3] if len(sys.argv) > 3 else "Sandra Bullock Daily"
